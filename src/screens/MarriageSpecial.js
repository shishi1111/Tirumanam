...
import SpecialDatas from "../components/SpecialDatas";

const MarriageSpecial = () => {
  const navigation = useNavigation();
  const [search, setSearch] = useState("");
  const [filteredDataSource, setFilteredDataSource] = useState([]);
  const [masterDataSource, setMasterDataSource] = useState([]);
  useEffect(() => {
    setFilteredDataSource(SpecialDatas);
    setMasterDataSource(SpecialDatas);
  }, []);
      ...
      <>
        <View
          style={{
            ...
            marginBottom: item?.title === "Train Ride" ? 80 : 10,
          }}
        >
          <TouchableOpacity
            activeOpacity={0.8}
...
export default MarriageSpecial;
