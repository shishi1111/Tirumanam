...
import DigitalDatas from "../components/DigitalDatas";

const MarriageDigital = () => {
  const navigation = useNavigation();
  const [search, setSearch] = useState("");
  const [filteredDataSource, setFilteredDataSource] = useState([]);
  const [masterDataSource, setMasterDataSource] = useState([]);
  useEffect(() => {
    setFilteredDataSource(DigitalDatas);
    setMasterDataSource(DigitalDatas);
  }, []);
      ...
      <>
        <View
          style={{
            ...
            marginBottom: item?.title === "3D Selfie Doll" ? 80 : 10,
          }}
        >
          <TouchableOpacity
            activeOpacity={0.8}
...
export default MarriageDigital;
