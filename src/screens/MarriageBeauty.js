...
import BeautyDatas from "../components/BeautyDatas";

const MarriageBeauty = () => {
  const navigation = useNavigation();
  const [search, setSearch] = useState("");
  const [filteredDataSource, setFilteredDataSource] = useState([]);
  const [masterDataSource, setMasterDataSource] = useState([]);
  useEffect(() => {
    setFilteredDataSource(BeautyDatas);
    setMasterDataSource(BeautyDatas);
  }, []);
      ...
      <>
        <View
          style={{
            ...
            marginBottom: item?.title === "Nail Art for Wedding" ? 80 : 10,
          }}
        >
          <TouchableOpacity
            activeOpacity={0.8}
